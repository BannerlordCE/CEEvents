...
#region Regular Event
        ...
        internal void CaptorConsequenceGameMenu(MenuCallbackArgs args)
        {
            ...
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.StripHero) && captiveHero != null)
            {
                ...
                {
                    CECustomHandler.ForceLogToFile("StripHero. Failed" + e);
                }
            }
...
#region Consequences
        ...
        private void ConsequenceCompanions()
        {
            ...
            {
                CECustomHandler.ForceLogToFile("ConsequenceCompanions. Failed" + e);
            }
        }
        ...
        private void ConsequenceRandomEventTriggerProgress(ref MenuCallbackArgs args)
        {
            ...
            try
            {
                foreach (TriggerEvent triggerEvent in _listedEvent.ProgressEvent.TriggerEvents)
                {
                    ...
                    try
                    {
                        weightedChance = _variableLoader.GetIntFromXML(!string.IsNullOrWhiteSpace(triggerEvent.EventWeight) ? triggerEvent.EventWeight : triggeredEvent.WeightedChanceOfOccurring);
                    }
        ...
        private void ConsequenceJoinParty()
        {
            ...
            try
            {
                if (_listedEvent.Captive.IsHero)
                {
                    ...
                    AddHeroToPartyAction.Apply(_listedEvent.Captive.HeroObject, PartyBase.MainParty.MobileParty);
                }
                else
                {
            ...
            {
                CECustomHandler.ForceLogToFile("Failure of JoinParty: " + e);
            }
        }
        ...
        private void ConsequenceWoundPrisoner(ref MenuCallbackArgs args)
        {
            ...
            // Wound Player
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.WoundCaptor))
                Hero.MainHero.MakeWounded(_listedEvent.Captive.HeroObject);
            // Wound All
            else if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.WoundAllPrisoners))
                _captor.CECaptorWoundPrisoners(args, PartyBase.MainParty.PrisonRoster.Count);
            // Wound Random
        ...
        private void ConsequenceKillPrisoner(ref MenuCallbackArgs args)
        {
            ...
            {
                if (_listedEvent.Captive.IsHero)
                    KillCharacterAction.ApplyByExecution(_listedEvent.Captive.HeroObject, Hero.MainHero);
                else
                    PartyBase.MainParty.PrisonRoster.AddToCounts(_listedEvent.Captive, -1);
            }
            ...
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.KillCaptor))
                _dynamics.CEKillPlayer(_listedEvent.Captive.HeroObject);
            // Kill All
            else if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.KillAllPrisoners))
                _captor.CECaptorKillPrisoners(args, PartyBase.MainParty.PrisonRoster.Count, true);
            // Kill Random
        ...
        private void ConsequenceRelease(ref MenuCallbackArgs args)
        {
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.Release))
            {
                ...
                {
                    CECustomHandler.ForceLogToFile("Consequence Release: " + e);
                }
            }


            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.ReleaseRandomPrisoners))
                _captor.CECaptorReleasePrisoners(args);
        ...
        private void ConsequenceEscape()
        {
            ...
            {
                if (_listedEvent.Captive.IsHero)
                    EndCaptivityAction.ApplyByEscape(_listedEvent.Captive.HeroObject);
                else
                    PartyBase.MainParty.PrisonRoster.AddToCounts(_listedEvent.Captive, -1);
            }
            catch (Exception e)
            {
                CECustomHandler.ForceLogToFile("Failure of Captor Escape: " + e);
            }
        }
        ...
        private void ConsequenceImpregnation(Hero captiveHero)
        {
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.ImpregnationHero))
            {
                try
                {
                    _impregnation.CaptivityImpregnationChance(captiveHero, !string.IsNullOrWhiteSpace(_option.PregnancyRiskModifier) ? _variableLoader.GetIntFromXML(_option.PregnancyRiskModifier) : _variableLoader.GetIntFromXML(_listedEvent.PregnancyRiskModifier));
                }
                catch (Exception)
                {
            ...
            else if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.ImpregnationRisk))
            {
                try
                {
                    _impregnation.CaptivityImpregnationChance(captiveHero, !string.IsNullOrWhiteSpace(_option.PregnancyRiskModifier) ? _variableLoader.GetIntFromXML(_option.PregnancyRiskModifier) : _variableLoader.GetIntFromXML(_listedEvent.PregnancyRiskModifier), false, false);
                }
                catch (Exception)
                {
                    CECustomHandler.LogToFile("Missing PregnancyRiskModifier");
            ...
            else if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.ImpregnationByPlayer))
            {
                try
                {
                    _impregnation.CaptivityImpregnationChance(captiveHero, !string.IsNullOrWhiteSpace(_option.PregnancyRiskModifier) ? _variableLoader.GetIntFromXML(_option.PregnancyRiskModifier) : _variableLoader.GetIntFromXML(_listedEvent.PregnancyRiskModifier), false, false, Hero.MainHero);
                }
                catch (Exception)
                {
                    CECustomHandler.LogToFile("Missing PregnancyRiskModifier");
        ...
        private void ConsequenceChangeHealth(Hero captiveHero)
        {
            ...
            try
            {
                captiveHero.HitPoints += !string.IsNullOrWhiteSpace(_option.HealthTotal) ? _variableLoader.GetIntFromXML(_option.HealthTotal) : _variableLoader.GetIntFromXML(_listedEvent.HealthTotal);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceRenown(Hero captiveHero)
        {
            ...
            try
            {
                _dynamics.RenownModifier(!string.IsNullOrWhiteSpace(_option.RenownTotal) ? _variableLoader.GetIntFromXML(_option.RenownTotal) : _variableLoader.GetIntFromXML(_listedEvent.RenownTotal), captiveHero);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceChangeGold(Hero captiveHero)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(_option.GoldTotal))
                    level = _variableLoader.GetIntFromXML(_option.GoldTotal);
                else if (!string.IsNullOrWhiteSpace(_listedEvent.GoldTotal))
                    level = _variableLoader.GetIntFromXML(_listedEvent.GoldTotal);
                else
        ...
        private void ConsequenceRelations(Hero captiveHero)
        {
            ...
            try
            {
                _dynamics.RelationsModifier(captiveHero, !string.IsNullOrWhiteSpace(_option.RelationTotal) ? _variableLoader.GetIntFromXML(_option.RelationTotal) : _variableLoader.GetIntFromXML(_listedEvent.RelationTotal), null, informationMessage && !noMessages, !informationMessage && !noMessages);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceProstitutionLevel(Hero captiveHero)
        {
            ...
            try
            {
                _dynamics.VictimProstitutionModifier(!string.IsNullOrWhiteSpace(_option.ProstitutionTotal) ? _variableLoader.GetIntFromXML(_option.ProstitutionTotal) : _variableLoader.GetIntFromXML(_listedEvent.ProstitutionTotal), captiveHero);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceProstitutionFlags(Hero captiveHero)
        {
            ...
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.AddProstitutionFlag))
                _dynamics.VictimProstitutionModifier(1, captiveHero, true, !informationMessage && !noMessages, informationMessage && !noMessages);
        ...
        private void ConsequenceSlaveryLevel(Hero captiveHero)
        {
            ...
            try
            {
                _dynamics.VictimSlaveryModifier(!string.IsNullOrWhiteSpace(_option.SlaveryTotal) ? _variableLoader.GetIntFromXML(_option.SlaveryTotal) : _variableLoader.GetIntFromXML(_listedEvent.SlaveryTotal), captiveHero);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceSlaveryFlags(Hero captiveHero)
        {
            ...
            if (_option.MultipleRestrictedListOfConsequences.Contains(RestrictedListOfConsequences.AddSlaveryFlag))
                _dynamics.VictimSlaveryModifier(1, captiveHero, true, !informationMessage && !noMessages, informationMessage && !noMessages);
        ...
        private void ConsequenceChangeMorale()
        {
            ...
            try
            {
                _dynamics.MoraleChange(!string.IsNullOrWhiteSpace(_option.MoraleTotal) ? _variableLoader.GetIntFromXML(_option.MoraleTotal) : _variableLoader.GetIntFromXML(_listedEvent.MoraleTotal), PartyBase.MainParty);
            }
            catch (Exception)
            {
        ...
        private void ConsequenceCaptorRenown()
        {
            ...
            try
            {
                _dynamics.RenownModifier(!string.IsNullOrWhiteSpace(_option.RenownTotal) ? _variableLoader.GetIntFromXML(_option.RenownTotal) : _variableLoader.GetIntFromXML(_listedEvent.RenownTotal), Hero.MainHero);
            }
            catch (Exception)
            {
                CECustomHandler.LogToFile("Missing RenownTotal");
        ...
        private void ConsequenceCaptorChangeGold()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(_option.CaptorGoldTotal))
                    level = _variableLoader.GetIntFromXML(_option.CaptorGoldTotal);
                else if (!string.IsNullOrWhiteSpace(_listedEvent.CaptorGoldTotal))
                    level = _variableLoader.GetIntFromXML(_listedEvent.CaptorGoldTotal);
                else
...
#region ReqFemaleCaptives
        ...
        private void ReqFemaleCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) <= _variableLoader.GetIntFromXML(_option.ReqFemaleCaptivesBelow)) return;
        ...
        private void ReqHeroFemaleCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) <= _variableLoader.GetIntFromXML(_option.ReqHeroFemaleCaptivesBelow)) return;
        ...
        private void ReqFemaleCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqFemaleCaptivesAbove)) return;
        ...
        private void ReqHeroFemaleCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroFemaleCaptivesAbove)) return;
...
#region ReqMaleCaptives
        ...
        private void ReqMaleCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqMaleCaptivesBelow)) return;
        ...
        private void ReqHeroMaleCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroMaleCaptivesBelow)) return;
        ...
        private void ReqMaleCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqMaleCaptivesAbove)) return;
            args.Tooltip = GameTexts.FindText("str_CE_captives_level", "low");
            args.IsEnabled = false;
        }
        ...
        private void ReqHeroMaleCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroMaleCaptivesAbove)) return;
...
#region ReqCaptives
        ...
        private void ReqCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Number) <= _variableLoader.GetIntFromXML(_option.ReqCaptivesBelow)) return;
        ...
        private void ReqHeroCaptivesBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) <= _variableLoader.GetIntFromXML(_option.ReqHeroCaptivesBelow)) return;
        ...
        private void ReqCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Number) >= _variableLoader.GetIntFromXML(_option.ReqCaptivesAbove)) return;
        ...
        private void ReqHeroCaptivesAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.PrisonRoster.Sum(troopRosterElement => troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroCaptivesAbove)) return;
...
#region ReqFemaleTroops
        ...
        private void ReqFemaleTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) <= _variableLoader.GetIntFromXML(_option.ReqFemaleTroopsBelow)) return;
        ...
        private void ReqHeroFemaleTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) <= _variableLoader.GetIntFromXML(_option.ReqHeroFemaleTroopsBelow)) return;
        ...
        private void ReqFemaleTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqFemaleTroopsAbove)) return;
        ...
        private void ReqHeroFemaleTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroFemaleTroopsAbove)) return;
...
#region ReqMaleTroops
        ...
        private void ReqMaleTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqMaleTroopsBelow)) return;
        ...
        private void ReqHeroMaleTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroMaleTroopsBelow)) return;
        ...
        private void ReqMaleTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqMaleTroopsAbove)) return;
        ...
        private void ReqHeroMaleTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if (PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => !troopRosterElement.Character.IsFemale && troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0) >= _variableLoader.GetIntFromXML(_option.ReqHeroMaleTroopsAbove)) return;
...
#region ReqTroops
        ...
        private void ReqTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if ((PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Number)) <= _variableLoader.GetIntFromXML(_option.ReqTroopsBelow)) return;
        ...
        private void ReqHeroTroopsBelow(ref MenuCallbackArgs args)
        {
            ...
            if ((PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0)) <= _variableLoader.GetIntFromXML(_option.ReqHeroTroopsBelow)) return;
        ...
        private void ReqTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if ((PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Number)) >= _variableLoader.GetIntFromXML(_option.ReqTroopsAbove)) return;
        ...
        private void ReqHeroTroopsAbove(ref MenuCallbackArgs args)
        {
            ...
            if ((PartyBase.MainParty.MemberRoster.Sum(troopRosterElement => troopRosterElement.Character.IsHero ? troopRosterElement.Number : 0)) >= _variableLoader.GetIntFromXML(_option.ReqHeroTroopsAbove)) return;
...
#region ReqHeroCaptorRelation
        ...
        private void ReqHeroCaptorRelation(ref MenuCallbackArgs args)
        {
            ...
            try
            {
                ReqHeroCaptorRelationBelow(ref args);
            }
        ...
        {
            if (string.IsNullOrWhiteSpace(_option.ReqHeroCaptorRelationBelow)) return;

            if (!(_listedEvent.Captive.HeroObject.GetRelationWithPlayer() > _variableLoader.GetFloatFromXML(_option.ReqHeroCaptorRelationBelow))) return;
